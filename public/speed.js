...
async function start() {
  ...
  try {
    ...
    conn.isHost = true;
    // `link:true`: aqui no hay codigo que dictar (la sala se comparte por enlace)
    // y tampoco secreto que proteger, asi que interesa un identificador largo e
    // inadivinable en vez de los 4 digitos publicos de `drop send`.
    wsSend({ t: 'host', v: 2, link: true });
  } catch (err) {
